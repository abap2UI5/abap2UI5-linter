 ...
 *   a( n v )          set an attribute (v: as written, b: a boolean, t: text)
 ...
 *
 * `a( t = <text> )` renders the value as LITERAL text: the builder applies
 * `escape_literal( )` to it, so a brace or a backslash in the value is shown
 * instead of parsed as a binding. It is the parameter for a value that
 * carries data — user input, a record field, a computed string — and
 * writing such a value through `v` instead is what
 * `unescaped-text-in-attribute` reports.
 *
...
export const VIEW_BUILDER = Object.freeze({
  ...
  text: 't',
  textFix: 'pass it as a( t = … ), which escapes it as literal text',
});
 ...
 *   textParam   the `a( t = )` parameter, or null when there is none
 */
export function dialectOf(content) {
  ...
  const dialect = Object.freeze({
    ...
    // the `a( t = )` parameter, same rule
    textParam: builders.map((b) => b.text).find(Boolean) ?? null,
